...
static void* thread_main() {
  ...
  int* data_address = p_side_data_address();
  assert(data_address == expected_address);

  printf("calling p_side_func_address=%p\n", p_side_func_address);
  func_t func = p_side_func_address();
  assert(expected_func_addr == func);
  assert(func() == 43);
...
int main() {
  ...
  expected_address = p_side_data_address();
